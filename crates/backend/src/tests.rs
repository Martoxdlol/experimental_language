...
fn native_object_contains_dwarf_debug_line() {
    ...
    assert!(
        has_line,
        "expected a DWARF debug_line section in the native object"
    );
}
...
fn shadowing_distinct_locals() {
    assert_eq!(
        run(
            "function f(): i64 { var x: i64 = 1; var y: i64 = { var x: i64 = 40; x + 1 }; x + y + 0 }",
            "f"
        ),
        42
    );
...
fn logical_short_circuit() {
    ...
    assert_eq!(
        run(
            "function f(): i64 { if (1 < 2) && (3 < 4) { 42 } else { 0 } }",
            "f"
        ),
        42
    );
    assert_eq!(
        run(
            "function f(): i64 { if (1 < 2) || (4 < 3) { 42 } else { 0 } }",
            "f"
        ),
        42
    );
...
fn string_escapes() {
    assert_eq!(
        run_str("function f(): str { \"a\\nb\\tc\" }", "f"),
        "a\nb\tc"
    );
    assert_eq!(
        run_str("function f(): str { \"quote: \\\"x\\\"\" }", "f"),
        "quote: \"x\""
    );
}
...
fn string_concat() {
    assert_eq!(
        run_str("function f(): str { \"foo\" + \"bar\" }", "f"),
        "foobar"
    );
    assert_eq!(
...
fn int_widen_and_narrow() {
    ...
    assert_eq!(
        run("function f(): i64 { var x: i32 = 300; x as i64 }", "f"),
        300
    );
    // i64 -> i8 truncation, then back to i64 sign-extended.
    assert_eq!(
        run(
            "function f(): i64 { var x: i64 = 300; (x as i8) as i64 }",
            "f"
        ),
        300i64 as i8 as i64
    );
...
fn str_substring_and_case() {
    assert_eq!(
        run_str("function f(): str { \"hello world\".substring(0, 5) }", "f"),
        "hello"
    );
    assert_eq!(
        run_str("function f(): str { \"abc\".to_upper() }", "f"),
        "ABC"
    );
    assert_eq!(
        run_str("function f(): str { \"  hi  \".trim() }", "f"),
        "hi"
    );
}
...
fn str_predicates() {
    assert_eq!(
        run("function f(): bool { \"hello\".contains(\"ell\") }", "f"),
        1
    );
    assert_eq!(
        run("function f(): bool { \"hello\".starts_with(\"he\") }", "f"),
        1
    );
    assert_eq!(
        run("function f(): bool { \"hello\".ends_with(\"lo\") }", "f"),
        1
    );
...
fn str_equality_by_content() {
    ...
    let src =
        "function f(): i64 { var a = \"x\" + \"y\"; var b = \"xy\"; if a == b { 42 } else { 0 } }";
    assert_eq!(run(src, "f"), 42);
...
fn hir_if_else_value_and_comparison() {
    assert_eq!(
        run_hir("function f(): i64 { if 1 < 2 { 42 } else { 0 } }", "f"),
        42
    );
    assert_eq!(
        run_hir("function f(): i64 { if 2 < 1 { 0 } else { 42 } }", "f"),
        42
    );
}
...
fn run_str_hir(src: &str, func: &str) -> String {
    ...
    assert!(
        hir_eligible_fns(&analysis) >= 1,
        "expected `{func}` to be HIR-eligible"
    );
    let jit = compile_hir(&analysis).expect("hir codegen");
    let bits = unsafe { jit.call_i64(func).expect("function present") };
    let p = bits as usize as *const runtime::LangStr;
...
fn hir_numeric_casts() {
    ...
    assert_eq!(
        run_hir("function f(): i64 { var x: i32 = 256; x as i64 }", "f"),
        256
    );
...
fn hir_tuple_literal_and_index() {
    assert_eq!(
        run_hir("function f(): i64 { var t = (40, 2); t.0 + t.1 }", "f"),
        42
    );
}
...
fn hir_union_is_tag_check() {
    ...
    assert_eq!(
        run_hir("function f(): bool { var x: i64 | str = 5; x is i64 }", "f"),
        1
    );
    assert_eq!(
...
fn hir_string_interpolation_builtin_holes() {
    assert_eq!(
        run_str_hir("function f(): str { var n: i64 = 42; \"n=$n\" }", "f"),
        "n=42"
    );
    assert_eq!(
        run_str_hir("function f(): str { var b: bool = true; \"$b\" }", "f"),
        "true"
    );
    assert_eq!(
        run_str_hir("function f(): str { \"hello, \" + \"world\" }", "f"),
        "hello, world"
    );
}
...
fn hir_list_literal_and_index_load() {
    assert_eq!(
        run_hir(
            "function f(): i64 { var xs = [10, 20, 12]; xs[0] + xs[1] + xs[2] }",
            "f"
        ),
        42
    );
...
fn hir_list_index_store() {
    assert_eq!(
        run_hir(
            "function f(): i64 { var xs = [1, 2, 3]; xs[0] = 40; xs[0] + xs[1] }",
            "f"
        ),
        42
    );
...
fn hir_map_literal_index_and_store() {
    assert_eq!(
        run_hir(
            "function f(): i64 { var m = { 1: 40, 2: 2 }; m[1] + m[2] }",
            "f"
        ),
        42
    );
    assert_eq!(
        run_hir(
            "function f(): i64 { var m = { 1: 0, 2: 2 }; m[1] = 40; m[1] + m[2] }",
            "f"
        ),
        42
    );
...
fn hir_for_list_accumulate() {
    let src =
        "function f(): i64 { var xs = [10, 20, 12]; var s = 0; for x in xs { s = s + x; } s }";
    assert_eq!(run_hir(src, "f"), 42);
}
...
fn hir_str_methods() {
    ...
    assert_eq!(
        run_hir("function f(): bool { \"hello\".contains(\"ell\") }", "f"),
        1
    );
    assert_eq!(
        run_hir("function f(): bool { \"hi\".starts_with(\"h\") }", "f"),
        1
    );
}
...
fn hir_map_methods() {
    ...
    assert_eq!(
        run_hir(
            "function f(): i64 { var m = { 1: 10, 2: 32 }; m.size() }",
            "f"
        ),
        2
    );
    assert_eq!(
        run_hir(
            "function f(): bool { var m = { 1: 10 }; m.contains(1) }",
            "f"
        ),
        1
    );
    assert_eq!(
        run_hir(
            "function f(): bool { var m = { 1: 10 }; m.contains(9) }",
            "f"
        ),
        0
    );
...
fn hir_closure_direct_call() {
    assert_eq!(
        run_hir(
            "function f(): i64 { var g = (x: i64): i64 => x + 1; g(41) }",
            "f"
        ),
        42
    );
...
fn hir_list_methods_nonclosure() {
    assert_eq!(
        run_hir(
            "function f(): i64 { var xs = [1, 2, 3]; xs.push(4); xs.size() }",
            "f"
        ),
        4
    );
    assert_eq!(
        run_hir(
            "function f(): bool { var xs: List<i64> = []; xs.is_empty() }",
            "f"
        ),
        1
    );
...
fn hir_collection_ctor_and_clone() {
    ...
    assert_eq!(
        run_hir(
            "function f(): i64 { var xs: List<i64> = List<i64>(); xs.push(42); xs[0] }",
            "f"
        ),
        42
    );
    assert_eq!(
        run_hir(
            "function f(): i64 { var m: Map<i64,i64> = Map<i64,i64>(); m[7] = 42; m[7] }",
            "f"
        ),
        42
    );
...
fn hir_numeric_intrinsics() {
    ...
    assert_eq!(
        run_hir("function f(): i64 { i32.MAX as i64 }", "f"),
        i32::MAX as i64
    );
    assert_eq!(
        run_hir("function f(): i64 { i64.wrapping_add(5, 37) }", "f"),
        42
    );
    assert_eq!(run_hir("function f(): bool { f64.is_nan(1.0) }", "f"), 0);
    assert_eq!(
        run_hir(
            "function f(): bool { var z: f64 = 0.0; f64.is_nan(z / z) }",
            "f"
        ),
        1
    );
...
#[test]
fn hir_thread_spawn_join_surface_is_await_only() {
    // The executor driver is not a language builtin. Keep this HIR smoke test
    // permissive so it does not reintroduce a user-visible `block_on` surface.
    let src = "function f(): i64 {\n\
    ...
    assert_eq!(
        a, b,
        "AST and HIR codegen disagree on Thread.spawn lowering"
    );
}
...
fn hir_shared_lock_and_try_lock() {
    ...
    assert!(analysis.errors.is_empty(), "sema: {:?}", analysis.errors);
    assert!(
        compile_hir(&analysis).is_ok(),
        "HIR codegen of async lock/try_lock failed"
    );
}
...
fn async_closure_state_layout_holds_captures_and_saves_them() {
    ...
    use crate::support::{ASYNC_INNER_OFF, BodyView, async_state_layout};
    use compiler::hir::ExprKind;
    ...
    for (l, _) in captures {
        assert!(
            captured.contains(l),
            "captured local {l:?} must be cell-backed"
        );
    }
    ...
    for cap in &cap_ids {
        ...
        assert_eq!(
            ct,
            crate::PTR,
            "a cell-backed capture occupies a pointer slot"
        );
        assert!(
