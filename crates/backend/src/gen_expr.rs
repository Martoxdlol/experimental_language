...
impl<'a, 'b, 'f, M: Module> FnGen<'a, 'b, 'f, M> {
    ...
    pub(crate) fn emit_vtable(&mut self, concrete: Ty, iface: Ty, span: Span) -> CgResult<Value> {
        ...
        let TyKind::Named {
            def: cdef,
            args: cargs,
        } = analysis.tcx.kind(concrete).clone()
        else {
        ...
        for m in &methods {
            ...
            let impl_def = (0..analysis.program.defs.len() as u32)
                .map(DefId)
                .find(|&d| {
                    let def = analysis.program.def(d);
    ...
    pub(crate) fn box_ready(&mut self, val: Option<Value>, out_ty: Ty) -> Value {
        ...
        let ready = self
            .call_intrinsic("lang_alloc", &[PTR], Some(PTR), &[rdesc])
            .expect("lang_alloc returns a pointer");
    ...
    pub(crate) fn emit_future_vtable(&mut self, poll_fid: FuncId) -> Value {
        ...
        self.module
            .define_data(data_id, &desc)
            .expect("define future vtable");
        let gv = self.module.declare_data_in_func(data_id, self.b.func);
        self.b.ins().global_value(PTR, gv)
    }

    /// Allocate and initialise a `Future<Out>` interface-object box for a state
    /// machine: `[vtable @0][data = state struct @8][drop_fn @16]`. The data
    /// pointer is GC-traced (offset 8). `drop_fn` is a generated
    /// `extern(state)` cleanup hook, or `0` when no prompt cancellation cleanup
    /// is needed.
    pub(crate) fn emit_future_box(
        &mut self,
        poll_fid: FuncId,
        drop_fid: Option<FuncId>,
        state: Value,
    ) -> Value {
        ...
        let drop_ptr = if let Some(drop_fid) = drop_fid {
            let fref = self.module.declare_func_in_func(drop_fid, self.b.func);
            self.b.ins().func_addr(PTR, fref)
        } else {
            self.b.ins().iconst(PTR, 0)
        };
        self.b.ins().store(MemFlags::trusted(), drop_ptr, bx, 16);
        bx
    }
    ...
    pub(crate) fn emit_await_suspend(
        &mut self,
        fut: Value,
        await_span: Span,
        out: Ty,
        ...
        let (state_n, poll_block, inner_off, self_val, ctx_val, pending_block, saves) = {
            let actx = self
                .async_ctx
                .as_ref()
                .ok_or_else(|| CodegenError::new(await_span, "`await` outside an async body"))?;
            ...
            let &(state_n, poll_block, _resume) =
                actx.awaits.get(&await_span).ok_or_else(|| {
                    CodegenError::new(
                        await_span,
        ...
        self.b
            .ins()
            .store(MemFlags::trusted(), fut, self_val, inner_off);
        ...
        let innerv = self
            .b
            .ins()
            .load(PTR, MemFlags::trusted(), self_val, inner_off);
        let r = self
            .emit_vtable_call(0, innerv, &[ctx_val], Some(PTR))?
    ...
    pub(crate) fn gen_float_to_int(&mut self, v: Value, ff: FloatTy, b: IntTy) -> Value {
        ...
        let (lo_v, hi_v) = match ff {
            FloatTy::F32 => (
                self.b.ins().f32const(lo as f32),
                self.b.ins().f32const(hi as f32),
            ),
        ...
        let it = int_clty(b);
        if signed {
            self.b.ins().fcvt_to_sint(it, v)
        } else {
            self.b.ins().fcvt_to_uint(it, v)
        }
    }
    ...
    pub(crate) fn guard_shift(&mut self, value: Value, amount: Value) {
        ...
        let too_big = self
            .b
            .ins()
            .icmp(IntCC::UnsignedGreaterThanOrEqual, amount, width_v);
    ...
    pub(crate) fn gen_str_compare(&mut self, op: BinaryOp, l: Value, r: Value) -> Value {
        ...
        match op {
            Eq => self
                .call_intrinsic("lang_str_eq", &[PTR, PTR], Some(types::I8), &[l, r])
                .expect("str_eq"),
            Ne => {
                let eq = self
                    .call_intrinsic("lang_str_eq", &[PTR, PTR], Some(types::I8), &[l, r])
            ...
            _ => {
                let cmp = self
                    .call_intrinsic("lang_str_cmp", &[PTR, PTR], Some(types::I64), &[l, r])
                    .expect("str_cmp");
    ...
    pub(crate) fn jump_to_merge(
        &mut self,
        merge: cranelift_codegen::ir::Block,
        val: Option<Value>,
        result_ct: Option<ClType>,
    ) -> CgResult<()> {
        if self.term {
        ...
        match (result_ct, val) {
            ...
            (Some(ct), None) => {
                ...
                let zero = self
                    .b
                    .ins()
                    .iconst(if ct.is_int() { ct } else { types::I64 }, 0);
