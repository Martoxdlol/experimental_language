...
fn diverges(s: &Stmt, never: compiler::ty::Ty) -> bool {
    match &s.kind {
        StmtKind::Expr(e) => {
...
fn scan_unreachable(b: &Block, never: compiler::ty::Ty, map: &SourceMap, out: &mut Vec<(Span, String)>) {
...
fn scan_expr_blocks(e: &Expr, never: compiler::ty::Ty, map: &SourceMap, out: &mut Vec<(Span, String)>) {
    use ExprKind as K;
    match &e.kind {
