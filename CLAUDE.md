...
Most internal identifiers (the `lang` CLI binary, the `lang-lsp` server, `lang_*` runtime symbols, the `LANG_GC` env var, the `source.lang` TextMate scope, the `lang` VS Code language id) are intentionally kept agnostic to the brand name — only user-visible surfaces (file extension, README, docs landing page, VS Code displayName/aliases, restart command title) carry the "Otter Fusion" name.
